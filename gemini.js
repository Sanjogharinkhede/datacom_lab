document.addEventListener("DOMContentLoaded", async function () {
  ...
  function getCategoryAndTopic(activeLink) {
    ...
    const category = section
      ...
      .textContent.replace("NMS", "NMS")
      .replace("DATACOM", "Datacom");
  ...
  async function generateAndDisplayContent(category, topic, topicText) {
    try {
      ...
      myHeaders.append(
        "x-goog-api-key",
        "AIzaSyChn5MaunVyrExauHPstceTwaxtOtdTI14"
      );
      ...
      // alert('Content generated for ' + topicText + '!.');
    } catch (error) {
  ...
  function displayGeneratedContent(info, topic) {
    ...
    card.innerHTML = `
            <div class="card">
                <div class="card-body">
                    ...
                    ${
                      info.features
                        ? `<span class="feature"><h6>Features</h6> ${info.features}</span>`
                        : ""
                    }
                    ${
                      info.benefits
                        ? `<h6>Benefits</h6><ul>${info.benefits
                            .map((b) => `<li>${b}</li>`)
                            .join("")}</ul>`
                        : ""
                    }
                    ${
                      info.disadvantages
                        ? `<h6>Disadvantages</h6><ul>${info.disadvantages
                            .map((d) => `<li>${d}</li>`)
                            .join("")}</ul>`
                        : ""
                    }
                    ${
                      info.commands
                        ? `<h6>Key Commands</h6><pre>${info.commands.join(
                            "\n"
                          )}</pre>`
                        : ""
                    }
    ...
    generatedContent.scrollTop =
      document.getElementById("imageGallery").scrollHeight;
    console.log(document.getElementById("imageGallery").scrollHeight);
  }
});
